...
void cleanupOldLogs(const std::string& logDir, int retentionDays)
{
	...
	try
	{
		...
		for (const auto& entry : std::filesystem::directory_iterator(logsPath))
		{
			...
			auto age = std::chrono::duration_cast<std::chrono::hours>(now - fileTimeSys);
...
int main()
{
	try
	{
		...
		auto rotatingSink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
			(logsDir / logFilename).string(), 1024 * 1024 * 5, 3);

		// Create logger with the file sink
		std::vector<spdlog::sink_ptr> sinks { rotatingSink };
		auto logger = std::make_shared<spdlog::logger>("workbench", sinks.begin(), sinks.end());
	...
	catch (const spdlog::spdlog_ex& ex)
	{
		...
		spdlog::warn("Failed to initialize file logging, using default: {}", ex.what());
	}
	...
	cleanupOldLogs(ConfigManager::instance().getLogsDir(), ConfigManager::instance().getConfig().ui.logRetentionDays);
