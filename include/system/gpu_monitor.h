 ...
 * nvidia-smi.
 *
 * Both VRAM usage (@ref get_stats()) and compute utilisation (@ref
 * get_load_stats()) are updated together during each call to @ref update(). All
 * public accessors are safe to call from multiple threads concurrently.
 */
class GpuMonitor
{
  public:
	 ...
	 *
	 * Parses @c nvidia-smi CSV output for every detected GPU. If @c nvidia-smi
	 * is unavailable the internal stats remain unchanged.
	 */
	void update();
	 ...
	 * by device index. Empty if no GPUs were detected.
	 */
